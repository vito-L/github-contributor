...
- **覆盖风险**：运行时会删除目标年份的所有已有贡献，重新生成
- **不可逆**：清除后无法恢复，建议先备份仓库
- **测试建议**：先用新名字测试（如 "TEST"），确认效果后再正式使用
- **建议用新仓库**：首次使用建议创建新仓库测试，确认效果后再应用到主仓库
...
⚠️ **注意**：设置 `overwrite: true` 会删除该年份所有已有贡献，重新生成。建议先用新仓库测试。
